...
MotorSpeed::MotorSpeed(QEI* encoder, double gyarRatio):
_encoder(encoder),
_gyarRatio(gyarRatio),
_thread()
{
}

void MotorSpeed::start() {
    _thread = make_unique<Thread>(MOTORSPEED_THREAD_PRIORITY, MOTORSPEED_THREAD_STACK_SIZE, nullptr, MOTORSPEED_THREAD_NAME);
